export function getWidth(el) {
  let isBorderBox =
    getComputedStyle(el).getPropertyValue('box-sizing') === 'border-box';
  ...
  if (isBorderBox) {
    ...
    const paddingRight = parseFloat(
      getComputedStyle(el).getPropertyValue('padding-right'),
    );
...
export function getHeight(el) {
  ...
  if (isBorderBox) {
    ...
    const borderBottom = parseFloat(
      getComputedStyle(el).getPropertyValue('border-bottom-width'),
    );
