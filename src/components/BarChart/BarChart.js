...
export default class BarChart extends Component {
  ...
  render() {
    ...
    return <div style={style} ref="barChart" />;
  }
    ...
    function barChart() {
      ...
      function chart(selection) {
        selection.each(function(data) {
          ...
          let mainGroup = layout
            .select('div.main-group')
            .style('top', margin.top + 'px')
            .style('left', margin.left + 1 + 'px');

          mainGroup
            .select('.bars-background')
          ...
          let barsEnterGroup = bars
            .enter()
            .append('div')
            .attr('class', function(d, i) {
              return i % 2 === 0 ? 'bar-group even' : 'bar-group odd';
            })
          ...
          if (activeBar) {
            ...
            changeToActive.classed('active', true);
          } else {
            const active = d3.select(this).selectAll('div.bar-group.active');
          ...
          bars
            ...
            .on('click', function(d) {
              ...
              const active = d3
                .select(chartElement)
                .selectAll('div.bar-group.active');
          ...
          bars.select('div.label').text(groupKey);

          bars.select('div.value').text(function(d) {
            return addCommas(d.current.count);
          });
      ...
      function redrawAxis() {
        ...
        let ticks = $el.selectAll('.axis > g.tick');
      ...
      function makeResponsive() {
        minBarHeight = 25;
        var newBarHeight = Math.round(
          (widgetHeight - margin.top - margin.bottom) / (datasetLength || 1),
        );
        newBarHeight =
          newBarHeight > maxBarHeight ? maxBarHeight : newBarHeight;
