...
export default class ScatterplotChart extends Component {
  ...
  render() {
    ...
    return <div style={style} ref="mapChart" />;
  }
  ...
  createChart() {
    ...
    let s =
      150 / Math.max((b[1][0] - b[0][0]) / width, (b[1][1] - b[0][1]) / height);
    ...
    this.updateChart = function(nextProps) {
      ...
      colorScale = logScale;
      colorScale.domain(colorDomain).range(['#424242', '#8DD1DB']);
    ...
    function mouseover(d) {
      ...
      let html = `<div class='marker-title'>${d.properties.name}</div>`;
