...
export default class ScatterplotChart extends Component {
  ...
  render() {
    ...
    return <div style={style} ref="scatterplotChart" />;
  }
  ...
  createChart() {
    ...
    this.updateChart = function(nextProps) {
      ...
      this.chart.width(getWidth(chartElement)).height(getHeight(chartElement));
    ...
    function scatterplotChart() {
      ...
      let clearBrushButton = d3
        .select(chartElement)
        .append('button')
        .text('Reset Zoom')
        .attr('class', 'clear-zoom')
        .style('display', 'none')
        .on('click', function() {
          ...
          d3
            ...
            .attr('transform', function(d) {
              return (
                'translate(' +
                x(d.current.metrics.injured.sum) +
                ',' +
                y(d.current.metrics.crashed.sum) +
                ')'
              );
            })
      ...
      function chart(selection) {
        selection.each(function(data) {
          ...
          const gEnter = svgEnter.append('g').attr('class', 'main-group');
          ...
          gEnter
            .select('g.brush')
            .insert('rect', 'rect.background')
            .attr('class', 'striped-background');

          svg
            .select('rect.striped-background')
            .attr('width', width)
            .attr('height', height);

          gEnter.select('rect.background').style('visibility', null);
          ...
          const scatterGroup = g
            .select('.scatterplot-group')
            .selectAll('.scatter-group')
            .data(data, groupKey);

          const scatterEnterGroup = scatterGroup
            .enter()
            .append('g')
          ...
          scatterEnterGroup.append('circle').attr('class', 'dot');

          scatterEnterGroup.append('circle').attr('class', 'pattern');
          ...
          scatterGroup
            .select('circle.dot')
            .attr('r', function(d) {
              return size(Math.abs(d.current.count));
            })
            .style('fill', '#00e8ff')
            .style('opacity', 1);
      ...
      function mouseover(d) {
        ...
        let html = `<div class="marker-title">${d.group}</div>`;
        for (var i = 0; i < fields.length; i++) {
          ...
          html += `<br /><div class="left">${
            fields[i]
          }</div><div class="right">${value}</div>`;
        }
      ...
      function mousemove() {
        ...
        if (
          parseInt(tooltip.style('left'), 10) +
            parseInt(tooltip.style('width'), 10) +
            parseInt(tooltip.style('padding'), 10) * 2 +
            parseInt(tooltip.style('border-radius'), 10) +
            parseInt(tooltip.style('border'), 10) >
          widgetWidth
        ) {
        ...
        if (
          parseInt(tooltip.style('top'), 10) +
            parseInt(tooltip.style('height'), 10) +
            parseInt(tooltip.style('padding'), 10) * 2 +
            parseInt(tooltip.style('border-radius'), 10) +
            parseInt(tooltip.style('border'), 10) >
          widgetHeight
      ...
      function updateAxisScales(data) {
        ...
        xDomainOrig = [
          minMetricValueX < 0 ? -absMaxMetricValueX : 0,
          absMaxMetricValueX,
        ];
        yDomainOrig = [
          minMetricValueY < 0 ? -absMaxMetricValueY : 0,
          absMaxMetricValueY,
        ];
      ...
      function brushmove() {
        ...
        d3
          ...
          .classed('selected', function(d) {
            var is_brushed =
              extent[0][0] <= x.invert(x(d.current.metrics.injured.sum)) &&
              x.invert(x(d.current.metrics.injured.sum)) <= extent[1][0] &&
              extent[0][1] <= y.invert(y(d.current.metrics.crashed.sum)) &&
              y.invert(y(d.current.metrics.crashed.sum)) <= extent[1][1];
            return is_brushed;
          });
      }
